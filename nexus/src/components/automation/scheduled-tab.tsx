...
/**
 * Scheduled Jobs tab body.
 *
 * Extracted from `src/app/(app)/dashboard/schedules/page.tsx` so the body
 * can be hosted inside the /dashboard/automation tabbed shell. The page
 * route owns the outer `p-6 space-y-6` wrapper plus the `<h1>` + subtitle.
 *
 * This tab keeps the feature chrome: action buttons (Refresh / New),
 * summary stats, schedule rows, and the editor modal.
 */
...
export function ScheduledTab() {
  ...
  return (
    <>
      {/* Action buttons — feature chrome that belongs with the tab. The
       * page-level <h1> + subtitle live on the /dashboard/schedules shell. */}
