...
/**
 * Script Chains tab body.
 *
 * Extracted from `src/app/(app)/dashboard/chains/page.tsx` so the body can
 * be hosted inside the /dashboard/automation tabbed shell. The page route
 * owns the outer `p-6 space-y-6` wrapper plus the `<h1>` + subtitle.
 *
 * This tab keeps the feature chrome: action buttons (Refresh / New chain),
 * summary stats, chain rows, and the editor modal.
 */
...
export function ChainsTab() {
  ...
  return (
    <>
       ...
       * page-level <h1> + subtitle live on the /dashboard/chains shell. */}
