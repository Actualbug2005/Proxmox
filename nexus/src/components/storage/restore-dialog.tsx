      ...
      <div className="studio-card p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-[var(--color-fg-subtle)] mt-0.5 font-mono break-all">{backup.volid}</p>
          </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className={inputCls}
            />
            <p className="text-xs text-[var(--color-fg-faint)] mt-1">Restores as a new {kind === 'qemu' ? 'VM' : 'CT'}; defaults to the next free ID.</p>
          </div>

          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Target storage</label>
        ...
        <div className="flex gap-3 justify-end mt-5">
          <button onClick={onClose} disabled={restoreM.isPending} className="px-4 py-2 text-sm text-[var(--color-fg-muted)] hover:text-white bg-[var(--color-overlay)] rounded-lg transition disabled:opacity-40">
            Cancel
          </button>
          <button
            onClick={submit}
