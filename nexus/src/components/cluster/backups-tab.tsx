 ...
 * Backups tab body.
 *
 * Extracted from `src/app/(app)/dashboard/cluster/backups/page.tsx` for the
 * Plan C Task 2 tabbed shell. The old /dashboard/cluster/backups route keeps
 * the page-level chrome (outer `p-6 space-y-6` + `<h1>`) and mounts this
 * component.
 *
 * Internal sub-tabs (`archive | jobs`) moved from local `useState` to
 * URL-driven `?sub=<id>` so deep-links like
 * `/dashboard/cluster?tab=backups&sub=jobs` work once Task 2 lands.
 */
...
type Sub = 'archive' | 'jobs';
const SUBS = ['archive', 'jobs'] as const;
...
export function BackupsTab() {
  ...
  const sub: Sub = isSub(sp.get('sub')) ? (sp.get('sub') as Sub) : 'archive';
  const setSub = (id: Sub) => {
