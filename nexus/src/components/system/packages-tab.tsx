...
/**
 * Node Packages tab body.
 *
 * Extracted from `src/app/(app)/dashboard/system/packages/page.tsx` for
 * the Plan D Task 2 tabbed shell. The /dashboard/system layout owns the
 * outer `p-6 space-y-6` wrapper AND the node-picker header; the old
 * route keeps only the page-level `<h1>` + subtitle chrome.
 *
 * Internal sub-tabs (`pve | system`) moved from local `useState` to
 * URL-driven `?sub=<id>` so deep-links like
 * `/dashboard/system?tab=packages&sub=system` will work once Task 2
 * lands. `scroll: false` on `router.replace` preserves scroll position
 * on toggle — the old useState toggle never jumped to top. Relative URL
 * so the same tab file renders correctly from both the standalone
 * /dashboard/system/packages route and the future tabbed shell.
 */
...
type Sub = 'pve' | 'system';
const SUBS = ['pve', 'system'] as const;
...
export function PackagesTab() {
  ...
  const sub: Sub = isSub(sp.get('sub')) ? (sp.get('sub') as Sub) : 'pve';
  const setSub = (id: Sub) => {
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-[var(--color-cta)] hover:bg-[var(--color-cta-hover)] text-[var(--color-cta-fg)] text-sm rounded-lg transition disabled:opacity-40"
            >
