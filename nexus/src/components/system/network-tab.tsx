...
/**
 * Node Network tab body.
 *
 * Extracted from `src/app/(app)/dashboard/system/network/page.tsx` for the
 ...
 * only the page-level `<h1>` + subtitle chrome.
 *
 * No URL sub-tabs — the body is a single two-pane layout (interface list
 * on the left, detail/edit/create form on the right). The action button
 * ("New Interface") used to live in the page-header row alongside the
 * title; now that the title moves to the thin shell, the action row is
 * owned by this tab so it stays adjacent to the list it mutates.
 */
