  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {isLoading && <Loader2 className="w-3.5 h-3.5 animate-spin text-gray-500" />}
      </div>

      {recent.length === 0 && !isLoading && (
        <p className="text-sm text-gray-600 py-4 text-center">No recent tasks</p>
      )}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-300 truncate">
                {task.type}
                {task.id && <span className="text-gray-500 ml-1">({task.id})</span>}
              </p>
              <p className="text-xs text-gray-600">{task.node} · {task.user}</p>
            </div>
