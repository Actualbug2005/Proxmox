  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Default user</label>
          <input
      ...
      <div>
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-[var(--color-fg-subtle)] hover:text-[var(--color-fg-secondary)]"
            aria-label={showPassword ? 'Hide password' : 'Show password'}
          >
            {showPassword ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
          </button>
        </div>
        <p className="text-xs text-[var(--color-fg-faint)] mt-1 flex items-center gap-1">
      ...
      {/* SSH keys */}
      <div>
        <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">SSH authorized keys</label>
        <textarea
      ...
      {/* NIC 0 */}
      <div className="rounded-lg border border-[var(--color-border-subtle)] p-3 space-y-3">
        <p className="text-[11px] uppercase tracking-widest text-[var(--color-fg-subtle)]">Network — eth0</p>

        <div className="grid grid-cols-2 gap-3">
          {/* IPv4 */}
          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv4</label>
            <select
          ...
          {/* IPv6 */}
          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv6</label>
            <select
        ...
        {value.nic0.ipv4Mode === 'static' && (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv4 address / prefix</label>
              <input
            ...
            <div>
              <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv4 gateway</label>
              <input
        ...
        {value.nic0.ipv6Mode === 'static' && (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv6 address / prefix</label>
              <input
            ...
            <div>
              <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">IPv6 gateway</label>
              <input
        ...
        {ipPreview && (
          <p className="text-[11px] text-[var(--color-fg-faint)] font-mono break-all">
            → ipconfig0 = <span className="text-[var(--color-fg-muted)]">{ipPreview}</span>
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Nameservers (optional)</label>
          <input
        ...
        <div>
          <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Search domain (optional)</label>
          <input
