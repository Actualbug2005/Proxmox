            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800/60">
                  ...
                  <th className="text-left px-4 py-3 text-xs text-zinc-500 font-medium">Groups</th>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.userid} className="border-b border-zinc-800/60/40 hover:bg-zinc-800/20">
                    <td className="px-4 py-3 font-mono text-zinc-200">{u.userid}</td>
                    <td className="px-4 py-3 text-xs text-zinc-400">{u.email ?? '—'}</td>
                    <td className="px-4 py-3 text-xs text-zinc-500 font-mono">{u.groups ?? '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex gap-0.5 justify-end">
                        <button onClick={() => setPwTarget(u)} disabled={!u.userid.endsWith('@pve') && !u.userid.endsWith('@pam')} className="p-1 text-zinc-300 hover:text-white bg-zinc-800 hover:bg-zinc-800 rounded-lg transition disabled:opacity-30" title="Change password (PVE/PAM realms only)"><KeyRound className="w-3 h-3" /></button>
                        <button onClick={() => setEdit(u)} className="p-1 text-zinc-300 hover:text-white bg-zinc-800 hover:bg-zinc-800 rounded-lg transition"><Pencil className="w-3 h-3" /></button>
                        <button onClick={() => setDeleteTarget(u)} disabled={u.userid === 'root@pam'} className="p-1 text-red-400 hover:text-red-300 bg-zinc-800 hover:bg-zinc-800 rounded-lg transition disabled:opacity-30" title={u.userid === 'root@pam' ? "Can't delete root" : 'Delete'}><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="bg-zinc-900 border border-zinc-800/60 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[1fr_120px] gap-3">
            ...
            <div>
              <label className="text-xs text-zinc-500 block mb-1">Realm</label>
          ...
          <div>
            <label className="text-xs text-zinc-500 block mb-1">Groups (comma-separated)</label>
            <input value={userGroups} onChange={(e) => setUserGroups(e.target.value)} placeholder="admins,auditors" className={inputCls} />
            {groups.length > 0 && <p className="text-xs text-zinc-600 mt-1 font-mono">available: {groups.map((g) => g.groupid).join(', ')}</p>}
          </div>
        ...
        <div className="flex gap-3 justify-end mt-5">
          ...
          <button onClick={submit} disabled={!userName || saveM.isPending} className="flex items-center gap-2 px-4 py-2 bg-zinc-100 hover:bg-white text-white text-sm rounded-lg transition disabled:opacity-40">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-zinc-900 border border-zinc-800/60 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div className="flex gap-3 justify-end">
          <button onClick={onClose} disabled={M.isPending} className="px-4 py-2 text-sm text-zinc-400 hover:text-white bg-zinc-800 rounded-lg transition disabled:opacity-40">Cancel</button>
          <button onClick={() => M.mutate()} disabled={!password || M.isPending} className="flex items-center gap-2 px-4 py-2 bg-zinc-100 hover:bg-white text-white text-sm rounded-lg transition disabled:opacity-40">
