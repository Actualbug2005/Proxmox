            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border-subtle)]">
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Path</th>
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Who</th>
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Role</th>
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Propagate</th>
                  <th className="text-right px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium"></th>
                </tr>
              </thead>
              <tbody>
                {acls.map((a, i) => (
                  <tr key={`${a.path}-${a.ugid}-${a.roleid}-${i}`} className="border-b border-zinc-800/40 hover:bg-zinc-800/20">
                    <td className="px-4 py-3 font-mono text-xs text-[var(--color-fg-secondary)]">{a.path}</td>
    ...
    <div className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto sm:py-8">
      <div className="studio-card p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Path</label>
          ...
          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Subject</label>
            <div className="flex gap-1 bg-[var(--color-overlay)] p-1 rounded-lg w-fit mb-2">
              <button onClick={() => setWho('user')} className={who === 'user' ? 'px-3 py-1 text-xs bg-[var(--color-overlay)] text-white rounded-md' : 'px-3 py-1 text-xs text-[var(--color-fg-subtle)] hover:text-[var(--color-fg-secondary)] rounded-md'}>User</button>
              <button onClick={() => setWho('group')} className={who === 'group' ? 'px-3 py-1 text-xs bg-[var(--color-overlay)] text-white rounded-md' : 'px-3 py-1 text-xs text-[var(--color-fg-subtle)] hover:text-[var(--color-fg-secondary)] rounded-md'}>Group</button>
            </div>
            {who === 'user' ? (
          ...
          <div>
            <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Role</label>
        ...
        <div className="flex gap-3 justify-end mt-5">
          ...
          <button onClick={() => saveM.mutate()} disabled={!path || !roleid || (who === 'user' ? !userid : !groupid) || saveM.isPending} className="flex items-center gap-2 px-4 py-2 bg-[var(--color-cta)] hover:bg-[var(--color-cta-hover)] text-[var(--color-cta-fg)] text-sm rounded-lg transition disabled:opacity-40">
