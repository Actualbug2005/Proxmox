            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border-subtle)]">
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Realm</th>
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Type</th>
                  <th className="text-left px-4 py-3 text-xs text-[var(--color-fg-subtle)] font-medium">Default</th>
              ...
              <tbody>
                {realms.map((r) => (
                  <tr key={r.realm} className="border-b border-zinc-800/40 hover:bg-zinc-800/20">
                    <td className="px-4 py-3 font-mono text-[var(--color-fg-secondary)]">{r.realm}</td>
                    <td className="px-4 py-3"><Badge variant="outline" className="text-xs">{r.type}</Badge></td>
                    <td className="px-4 py-3 text-xs text-[var(--color-fg-muted)]">{(r.default ?? false) ? 'yes' : ''}</td>
                    <td className="px-4 py-3 text-xs text-[var(--color-fg-muted)]">{r.comment ?? '—'}</td>
      ...
      <div className="studio-card p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[1fr_140px] gap-3">
            <div>
              <label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Realm ID</label>
          ...
          {(type === 'ldap' || type === 'ad') && (
            <>
              ...
              <h4 className="text-xs uppercase text-[var(--color-fg-faint)] font-medium tracking-widest">Directory</h4>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Server 1</label><input value={server1} onChange={(e) => setServer1(e.target.value)} className={inputCls} /></div>
                <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Server 2 (optional)</label><input value={server2} onChange={(e) => setServer2(e.target.value)} className={inputCls} /></div>
              </div>
              <div className="grid grid-cols-[1fr_100px] gap-3">
                <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Base DN</label><input value={baseDn} onChange={(e) => setBaseDn(e.target.value)} placeholder="DC=example,DC=com" className={inputCls + ' font-mono'} /></div>
                <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Port</label><input type="number" value={port} onChange={(e) => setPort(e.target.value)} className={inputCls} /></div>
              </div>
              <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">User attribute</label><input value={userAttr} onChange={(e) => setUserAttr(e.target.value)} placeholder={type === 'ad' ? 'sAMAccountName' : 'uid'} className={inputCls} /></div>
              <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Bind DN (optional)</label><input value={bindDn} onChange={(e) => setBindDn(e.target.value)} placeholder="CN=nexus,OU=Service Accounts,DC=example,DC=com" className={inputCls + ' font-mono text-xs'} /></div>
              <label className="flex items-center gap-2 text-sm text-[var(--color-fg-secondary)] cursor-pointer">
          ...
          {type === 'openid' && (
            <>
              <div className="pt-2 border-t border-[var(--color-border-subtle)]" />
              <h4 className="text-xs uppercase text-[var(--color-fg-faint)] font-medium tracking-widest">OpenID Connect</h4>
              <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Issuer URL</label><input value={issuer} onChange={(e) => setIssuer(e.target.value)} placeholder="https://id.example.com" className={inputCls} /></div>
              <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Client ID</label><input value={clientId} onChange={(e) => setClientId(e.target.value)} className={inputCls} /></div>
              <div><label className="text-xs text-[var(--color-fg-subtle)] block mb-1">Client secret</label><input value={clientKey} onChange={(e) => setClientKey(e.target.value)} className={inputCls} /></div>
              <label className="flex items-center gap-2 text-sm text-[var(--color-fg-secondary)] cursor-pointer">
