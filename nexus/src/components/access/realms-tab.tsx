            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Realm</th>
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Type</th>
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Default</th>
                  ...
                  <tr key={r.realm} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex gap-0.5 justify-end">
                        {canSync(r) && (
                          <button onClick={() => syncM.mutate(r)} disabled={syncM.isPending} className="p-1 text-blue-400 hover:text-blue-300 bg-gray-800 hover:bg-gray-700 rounded-lg transition" title="Sync users"><RefreshCw className="w-3 h-3" /></button>
                        )}
                        <button onClick={() => setEdit(r)} className="p-1 text-gray-300 hover:text-white bg-gray-800 hover:bg-gray-700 rounded-lg transition"><Pencil className="w-3 h-3" /></button>
                        <button onClick={() => setDeleteTarget(r)} disabled={!canDelete(r)} className="p-1 text-red-400 hover:text-red-300 bg-gray-800 hover:bg-gray-700 rounded-lg transition disabled:opacity-30" title={!canDelete(r) ? 'Cannot delete built-in realm' : 'Delete'}><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[1fr_140px] gap-3">
            <div>
              <label className="text-xs text-gray-500 block mb-1">Realm ID</label>
          ...
          {(type === 'ldap' || type === 'ad') && (
            <>
              ...
              <h4 className="text-xs uppercase text-gray-600 font-medium tracking-wide">Directory</h4>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-gray-500 block mb-1">Server 1</label><input value={server1} onChange={(e) => setServer1(e.target.value)} className={inputCls} /></div>
                <div><label className="text-xs text-gray-500 block mb-1">Server 2 (optional)</label><input value={server2} onChange={(e) => setServer2(e.target.value)} className={inputCls} /></div>
              </div>
              <div className="grid grid-cols-[1fr_100px] gap-3">
                <div><label className="text-xs text-gray-500 block mb-1">Base DN</label><input value={baseDn} onChange={(e) => setBaseDn(e.target.value)} placeholder="DC=example,DC=com" className={inputCls + ' font-mono'} /></div>
                <div><label className="text-xs text-gray-500 block mb-1">Port</label><input type="number" value={port} onChange={(e) => setPort(e.target.value)} className={inputCls} /></div>
              </div>
              <div><label className="text-xs text-gray-500 block mb-1">User attribute</label><input value={userAttr} onChange={(e) => setUserAttr(e.target.value)} placeholder={type === 'ad' ? 'sAMAccountName' : 'uid'} className={inputCls} /></div>
              <div><label className="text-xs text-gray-500 block mb-1">Bind DN (optional)</label><input value={bindDn} onChange={(e) => setBindDn(e.target.value)} placeholder="CN=nexus,OU=Service Accounts,DC=example,DC=com" className={inputCls + ' font-mono text-xs'} /></div>
              <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
          ...
          {type === 'openid' && (
            <>
              <div className="pt-2 border-t border-gray-800" />
              <h4 className="text-xs uppercase text-gray-600 font-medium tracking-wide">OpenID Connect</h4>
              <div><label className="text-xs text-gray-500 block mb-1">Issuer URL</label><input value={issuer} onChange={(e) => setIssuer(e.target.value)} placeholder="https://id.example.com" className={inputCls} /></div>
              <div><label className="text-xs text-gray-500 block mb-1">Client ID</label><input value={clientId} onChange={(e) => setClientId(e.target.value)} className={inputCls} /></div>
              <div><label className="text-xs text-gray-500 block mb-1">Client secret</label><input value={clientKey} onChange={(e) => setClientKey(e.target.value)} className={inputCls} /></div>
              <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
        ...
        <div className="flex gap-3 justify-end mt-5">
          ...
          <button onClick={submit} disabled={!realm || saveM.isPending} className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm rounded-lg transition disabled:opacity-40">
