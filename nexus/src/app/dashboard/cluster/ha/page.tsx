  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Quorum, HA resources, and HA groups</p>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">SID</th>
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Desired</th>
                    ...
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Restart / Relocate</th>
                ...
                <tbody>
                  {resources.map((r) => {
                    ...
                    return (
                      <tr key={r.sid} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                        <td className="px-4 py-2 font-mono text-gray-200">{r.sid}</td>
                        <td className="px-4 py-2"><Badge variant={stateVariant(r.state)} className="text-xs">{r.state}</Badge></td>
                        <td className="px-4 py-2"><Badge variant={stateVariant(current)} className="text-xs">{current ?? 'unknown'}</Badge></td>
                        <td className="px-4 py-2 text-xs text-gray-400 font-mono">{r.group ?? '—'}</td>
                        ...
                        <td className="px-4 py-2 text-right">
                          <div className="flex gap-1 justify-end">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1.5 px-2.5 py-1 text-xs text-yellow-400 hover:text-yellow-300 bg-gray-800 hover:bg-gray-700 rounded-lg transition"
                            ...
                            <button
                              ...
                              className="p-1 text-xs text-gray-300 hover:text-white bg-gray-800 hover:bg-gray-700 rounded-lg transition"
                              title="Edit"
                            >
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Group</th>
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Nodes (priority)</th>
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Flags</th>
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Used by</th>
                    <th className="text-right px-4 py-2.5 text-xs text-gray-500 font-medium"></th>
                  </tr>
                </thead>
                <tbody>
                  {groups.map((g) => (
                    <tr key={g.group} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                      <td className="px-4 py-2 font-mono text-gray-200">{g.group}</td>
                      <td className="px-4 py-2 font-mono text-xs text-gray-400">{g.nodes}</td>
                      ...
                      <td className="px-4 py-2 text-xs text-gray-500">{groupRefs(g.group)} resources</td>
                      <td className="px-4 py-2 text-right">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => setEditGroup(g)} className="p-1 text-xs text-gray-300 hover:text-white bg-gray-800 hover:bg-gray-700 rounded-lg transition" title="Edit">
                            <Pencil className="w-3 h-3" />
                          </button>
                          <button onClick={() => setDeleteGroup(g)} className="p-1 text-xs text-red-400 hover:text-red-300 bg-gray-800 hover:bg-gray-700 rounded-lg transition" title="Delete">
                            <Trash2 className="w-3 h-3" />
                          </button>
                        </div>
                      </td>
                ...
                <tbody>
                  {haStatus.map((s) => (
                    <tr key={s.id} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                      <td className="px-4 py-2 font-mono text-xs text-gray-200">{s.id}</td>
                      <td className="px-4 py-2 text-xs text-gray-400">{s.type}</td>
                      <td className="px-4 py-2 text-xs text-gray-400 font-mono">{s.node ?? '—'}</td>
                      <td className="px-4 py-2"><Badge variant={stateVariant(s.state)} className="text-xs">{s.state ?? '—'}</Badge></td>
                      <td className="px-4 py-2 text-xs text-gray-500">{s.request_state ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
