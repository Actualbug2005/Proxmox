...
function ScriptCard({
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800/60 hover:border-zinc-800/60 rounded-lg p-4 transition">
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-zinc-800/60 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {script.resources.cpu && (
                <div className="text-xs">
                  ...
                  <span className="text-zinc-400">{script.resources.cpu} cores</span>
                </div>
              )}
              {script.resources.ram && (
                <div className="text-xs">
                  <span className="text-zinc-600">RAM:</span>{' '}
                  <span className="text-zinc-400">{script.resources.ram} MB</span>
                </div>
              )}
              {script.resources.hdd && (
                <div className="text-xs">
                  <span className="text-zinc-600">Disk:</span>{' '}
                  <span className="text-zinc-400">{script.resources.hdd}</span>
                </div>
              )}
            </div>
          )}
          ...
          <p className="text-xs text-zinc-600 font-mono break-all">{script.scriptUrl}</p>
        </div>
      )}
    </div>
  );
...
export default function ScriptsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full bg-zinc-900 border border-zinc-800/60 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-orange-500 focus:border-orange-500"
          />
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                categoryFilter === cat
                  ...
                  : 'text-zinc-600 hover:text-zinc-400 bg-zinc-900 border border-zinc-800/60',
              )}
            >
              {cat}
            </button>
      ...
      {!isLoading && !isError && (
        <>
          <p className="text-xs text-zinc-600">{filtered.length} results</p>
      ...
      {runModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-800/60 rounded-lg p-6 w-full max-w-md">
            ...
            {runState?.status !== 'success' && runState?.status !== 'error' ? (
              <>
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="w-full bg-zinc-800 border border-zinc-800/60 rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-orange-500 appearance-none"
                    >
                      {nodes.map((n) => (
                ...
                <div className="mb-4 p-3 bg-zinc-800 rounded-lg">
                  <p className="text-xs text-zinc-500 mb-1">Command to run:</p>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => { setRunModal(null); setRunState(null); }}
                    className="flex-1 px-4 py-2 bg-zinc-800 hover:bg-zinc-800 text-zinc-300 text-sm rounded-lg transition"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <div className="text-center py-4">
                ...
                {runState.upid && (
                  <p className="text-xs text-zinc-500 font-mono break-all">UPID: {runState.upid}</p>
                )}
                ...
                <button
                  onClick={() => { setRunModal(null); setRunState(null); }}
                  className="mt-4 px-4 py-2 bg-zinc-800 hover:bg-zinc-800 text-zinc-300 text-sm rounded-lg transition"
                >
                  Close
                </button>
              </div>
              ...
              <div className="text-center py-4">
                ...
                <p className="text-xs text-zinc-500">{runState.message}</p>
