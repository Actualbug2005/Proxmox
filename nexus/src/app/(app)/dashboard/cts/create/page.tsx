...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-xs text-[var(--color-fg-subtle)] block mb-1.5">{label}</label>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-2 border-t border-[var(--color-border-subtle)]">
        <p className="text-xs font-medium text-[var(--color-fg-muted)] mb-3">Root Disk</p>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Storage">
...
function StepConfirm({ state }: { state: WizardState }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm text-[var(--color-fg-muted)] mb-4">Review your settings before creating the container.</p>
      {rows.map(([label, value]) => (
        <div key={label} className="flex justify-between py-2 border-b border-[var(--color-border-subtle)] last:border-0">
          <span className="text-xs text-[var(--color-fg-subtle)]">{label}</span>
          <span className="text-sm text-[var(--color-fg-secondary)] font-mono">{value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => router.push('/dashboard/cts')} className="text-[var(--color-fg-subtle)] hover:text-[var(--color-fg-secondary)] transition">
          ...
          <div key={label} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center">
              <div className={cn(
                ...
                'bg-[var(--color-overlay)] text-[var(--color-fg-faint)]',
              )}>
                {i < step ? <Check className="w-3.5 h-3.5" /> : i + 1}
              </div>
              <span className={cn('text-xs mt-1 whitespace-nowrap', i === step ? 'text-indigo-400' : 'text-[var(--color-fg-faint)]')}>
                {label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={cn('flex-1 h-px mx-2 mb-4', i < step ? 'bg-zinc-100/40' : 'bg-[var(--color-overlay)]')} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[var(--color-overlay)] hover:bg-[var(--color-overlay)] text-[var(--color-fg-secondary)] text-sm rounded-lg transition"
        >
        ...
        {isLast ? (
          <button
            ...
            className="flex items-center gap-2 px-6 py-2 bg-[var(--color-cta)] hover:bg-[var(--color-cta-hover)] text-[var(--color-cta-fg)] text-sm font-medium rounded-lg transition disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={!canNext()}
            className="flex items-center gap-2 px-4 py-2 bg-[var(--color-cta)] hover:bg-[var(--color-cta-hover)] text-[var(--color-cta-fg)] text-sm font-medium rounded-lg transition disabled:opacity-50"
          >
            Next
